...
## Functions for column renaming and get hash
def column_renamer(df,suffix, append):
    ...
    return df.toDF(*new_culumn_names)
...
## this is the delta tabe dlt bring in max_date_data
...
def get_max_date_df():
    max_date_df = spark.readStream.format("cloudFiles").option("cloudFiles.format","json").schema(max_data_schema).load('dbfs:/mnt/data/BEADAnalytics/Bronze/bdc_max_date/')
