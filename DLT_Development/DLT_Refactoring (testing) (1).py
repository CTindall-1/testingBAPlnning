...
spark.conf.set("fs.azure.account.key.nbamdatabricksstorage.blob.core.windows.net","qVkcpRrIt6YUZ6vixUJmlIKZbMZDpbnjuYLQPb015RUBihbsb+RiBZeyqtGjS8t79zuiVJRWKskO+AStzKXJRQ==")

# COMMAND ----------

# # goal is to remove the need for the scheduler piece and set this up as the autoloader table essentially

# df_pandas = spark.read.format('csv').options(header='true').load('dbfs:/mnt/data/fcc_nbm_api/data/063022_012223/bdc_01_Cable_fixed_broadband_063022.csv').toPandas()
# df_pandas.display()

# COMMAND ----------

# # this table brings in the source _file name
# from pyspark.sql.functions import col

# df_testing = spark.read.format('csv').options(header='true').load('dbfs:/mnt/data/fcc_nbm_api/data/063022_012223/bdc_01_Cable_fixed_broadband_063022.csv').select("*", col("_metadata.file_path").alias("source_file_name")).toPandas()
...
# %sql
# SELECT * FROM ba_planning_tool.authoritative_bdc
...
# df_testing['testDate'] = df_testing['source_file_name'].apply(extract_Parse_Date)
...
bdc_schema = StructType([StructField('frn', StringType(), True),
                            ...
                            StructField('testData', DateType(), True)])
...
#     max_data_df = dlt.read_stream("max_date_data").withWatermark('version_time', "5 Seconds")
...
# %sql
# CREATE TABLE testAutoloader;

# COMMAND ----------

# @dlt.create_table(comment="",
#                   temporary=True
#                   )
# def raw_autoloader():
#   return (
...
#       .load('/dbfs/mnt/data/BEADAnalytics/playground/dlt_test/063022_012223')
#       .writeStream
#       .format("delta")
#       .option("inferSchema","true")
#       .option("cloudFiles.useNotifications", True) 
#       .trigger(once=True)
#       .saveAsTable("testAutoLoader")
#   )
# # def autoloader_Test():
# #     return(
# #         spark.readStream.format("clou")
# #             .option("cloudFiles.format", "csv")
# #             .option("cloudFiles.useNotifications","true")
# #             .option("cloudFiles.includeSubdirectories", "true")
# #             .load('/mnt/data/BEADAnalytics/playground/dlt_test/')
# #             .writeStream
# #             .option("inferSchema","true")
# #             .trigger(once=True)
# #             .table(dbdemos.test)
...
# def extract_Parse_Date(path):
#     date_string = path.split('/')[-2]
#     date_string = date_string.split('_')[0]
#     parsed_date = datetime.strptime(date_string, '%m%d%y')
#     formatted_date = parsed_date.strftime('%m%d%y')
#     return formatted_date

# extract_Parse_Date_udf = udf(extract_Parse_Date, DateType())
...
#             .option("cloudFiles.format", "csv")
#             .schema(bdc_schema)
#             .load("wasbs://nbamdatabricksstorage@nbamdatabricksstorage.blob.core.windows.net/BEADAnalytics/dlt_test/data/*/*.csv")
#             .withColumn("file_path",col("_metadata.file_path"))
#             .withColumn("file_name",col("_metadata.file_path"))
#             .withColumn("file_modification_time",col("_metadata.file_modification_time"))
#             .withColumn("version_date", extract_Parse_Date_udf(col("file_path")))
#     )
...
#   .option("cloudFiles.format", "csv") \
#   .schema(bdc_schema) \
#   .load("dbfs:/mnt/data/BEADAnalytics/dlt_test/063022_012223")\
#   .display()
...
#     .option("cloudFiles.format", "csv") 
#     .schema(bdc_schema)
#     .load("abfss://nbamdatabricksstorage@nbamdatabricksstorage.blob.core.windows.net/nbamdatabricksstorage/BEADAnalytics/playground/dlt_test/063022_012223")
#     .display()
# )
...
# df=spark.read.format("csv").load("dbfs:/mnt/data/BEADAnalytics/dlt_test/063022_012223/bdc_01_Cable_fixed_broadband_063022.csv").display()

# COMMAND ----------

def extract_Parse_Date(path):
    date_string = path.split('/')[-2]
    date_string = date_string.split('_')[1]
...
from pyspark.sql.types import DateType
from datetime import datetime
date= extract_Parse_Date('dbfs:/mnt/data/fcc_nbm_api/data/063022_012223/bdc_01_Cable_fixed_broadband_063022.csv')
...
## pull from fabric table instead of bdc_history
...
def get_max_date_df():
    max_date_df = spark.sql("SELECT MAX(version_date) AS max_date FROM LIVE.bronze_Version_Data")
...
@dlt.table(
    name='authoritative_bdc',
    table_properties = {"quality": "silver"}
)

def process_bdc_merge():
    bdc_df = dlt.read("bdc_history").withWatermark('bdc_time', "2 hours").limit(100)
    current_df = dlt.read("bronze_Version_Data").withWatermark('time', "2 hours").limit(100)
    lookup = dlt.read("location_id_to_geoid").withWatermark('bdc_time', "2 hours")
    max_data_df = dlt.read("max_date_data").withWatermark('max_date_time', "2 hours")
    
    max_data_df = max_data_df.select('version_date', 'max_date_time').withColumn('temp1', lit('temp'))\
                                                                     .withColumnRenamed('version_date', 'effective_date')\
                                                                     .withWatermark('max_date_time', '2 hours')


    current_df = current_df.join(max_data_df, col('time')==col('max_date_time'), how='left')
    current_df = current_df.withColumn('effective_date', to_date(col('effective_date'), DATE_FORMAT))
    ...
    df_history_open_hash = column_renamer(get_hash(df_history_open, type2_cols), suffix="_history", append=True)
    df_current_hash = column_renamer(get_hash(current_df, type2_cols), suffix="_current", append=True)
    df_current_hash = dropDupeCols(df_current_hash)
    print(df_current_hash.columns)
    df_merged = df_history_open_hash\
                .join(df_current_hash, ((col("location_id_current")==col("location_id_history"))&
                                        (col('frn_current')==col('frn_history'))&
                                        (col('brand_name_current')==col('brand_name_history'))&
    ...
    df_merged = dropDupeCols(df_merged)
    df_final = df_merged.select([col for col in df_merged.columns if col.endswith('_current')])
    return df_final


    ## add tag add in filters it outputs the table without the _current 

# COMMAND ----------

    max_date_df = spark.sql("SELECT * FROM ba_planning_tool.authoritative_bdc WHERE state_geoid==22")

    display(max_date_df)


    Lat_long_LA = spark.sql("SELECT * FROM ba_planning_tool.state_lookup_table")
    display(Lat_long_LA)
